...
// Global logic for SPA View Switching
window.navigate = function(viewId) {
    ...
    document.querySelectorAll('.navbar-nav .nav-link').forEach(link => {
        ...
        if (link.getAttribute('href') === '#' + viewId || link.getAttribute('href').includes(viewId)) {
...
window.addEventListener('popstate', function() {
    ...
    } else if (!hash || hash === 'home' || hash === 'view-home') {
        window.navigate('view-home');
    }
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (initialHash && document.getElementById(initialHash) && document.getElementById(initialHash).classList.contains('page-view')) {
        window.navigate(initialHash);
    } else if (initialHash) {
        window.navigate('view-home');
        setTimeout(() => {
    ...
    // Smooth scrolling for navigation links (ignoring view links)
    const navLinks = document.querySelectorAll('a[href^="#"]:not([onclick])');
    navLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            const targetId = this.getAttribute('href');
            // If it's just a view hash that doesn't use onclick, let's navigate
            if (targetId.startsWith('#view-')) {
                e.preventDefault();
                window.navigate(targetId.substring(1));
                return;
            }
            ...
            if (targetSection) {
                // Ensure we are in home view if navigating to section
                window.navigate('view-home');
